# Environment Descriptor: mossy_waste_plains

- descriptor_id: `DESC_ENV_mossy_waste_plains`
- canonical_id: `mossy_waste_plains`
- status: `canonical`
- entity_type: `environment`

## Supported Fields

- camera_friendly_landmarks:
  - Vast mossy waste
  - trackless Martian plains
  - desolate landscape
  - endless green-grey moss
  - arid wasteland.
- chapter_mentions:
  - CH017
- environment_type: Secondary (Transit/Survival)
- layout: Endless horizon, featureless terrain, absence of water sources.
- lighting: Harsh, exposed, desolate, sense of extreme exhaustion and heat.
- mood: Harsh, exposed, desolate, sense of extreme exhaustion and heat.
- recurring_anchors:
  - Vast mossy waste
  - trackless Martian plains
  - desolate landscape
  - endless green-grey moss
  - arid wasteland.

## Generated Fields


## Coverage

- chapter_mentions: CH017
- scene_mentions: (none)
- shot_mentions: (none)

## Evidence Summary

- Endless horizon, featureless terrain, absence of water sources.
- Harsh, exposed, desolate, sense of extreme exhaustion and heat.
- Vast mossy waste
- trackless Martian plains
- desolate landscape
- endless green-grey moss
- arid wasteland.
- Secondary (Transit/Survival)

## Review

- (none)

## Metadata

- artifact_id: `DESC_ENV_mossy_waste_plains`
- status: `generated`
- source_fingerprint: `ace6b0d1f12419551657089256669527ada3433a`
- created_at_utc: `2026-04-21T05:20:52.798438+00:00`
- updated_at_utc: `2026-04-21T05:20:52.798438+00:00`
