# Environment Descriptor: martian_sleeping_chamber

- descriptor_id: `DESC_ENV_martian_sleeping_chamber`
- canonical_id: `martian_sleeping_chamber`
- status: `canonical`
- entity_type: `environment`

## Supported Fields

- camera_friendly_landmarks:
  - Decorated surfaces/walls
  - Small ten-legged Martian creatures (pony-like, frog heads, tusks)
- chapter_mentions:
  - CH004
- environment_type: Secondary Setting / Martian Interior
- layout: Interior room featuring decorated walls; presence of small Martian creatures.
- lighting: Intimate but alien.
- mood: Domestic Martian atmosphere; intimate yet exotic.
- recurring_anchors:
  - Decorated surfaces/walls
  - Small ten-legged Martian creatures (pony-like, frog heads, tusks)

## Generated Fields

- architecture: ancient stone and ceremonial architecture
- depth_cues: strong scale contrast and layered architectural depth
- foreground_midground_background: anchored foreground details, readable midground action, deep background structures
- geography: dry open Martian terrain
- materials: stone, metal, and weathered architectural surfaces
- pathways: broad processional paths and stairs
- scale: monumental scale
- weather_or_atmosphere: dry thin air with still heat

## Coverage

- chapter_mentions: CH004
- scene_mentions: (none)
- shot_mentions: (none)

## Evidence Summary

- An intimate, ancient Martian sleeping chamber characterized by decorated surfaces and walls. The space exhibits a sense of alien domesticity, populated by small, exotic fauna including ten-legged creatures described as pony-like with frog heads and tusks.
- Interior room featuring decorated walls; presence of small Martian creatures.
- Intimate but alien.
- Domestic Martian atmosphere; intimate yet exotic.
- Decorated surfaces/walls
- Small ten-legged Martian creatures (pony-like, frog heads, tusks)
- Must maintain an "alien domesticity" aesthetic.
- Lighting must remain intimate but distinctly non-human.

## Review

- generated_field_placeholder_scale
- generated_field_placeholder_geography
- generated_field_placeholder_architecture
- generated_field_placeholder_pathways
- generated_field_placeholder_materials
- generated_field_placeholder_weather_or_atmosphere
- generated_field_placeholder_foreground_midground_background
- generated_field_placeholder_depth_cues

## Metadata

- artifact_id: `DESC_ENV_martian_sleeping_chamber`
- status: `generated`
- source_fingerprint: `cf52eecdde150c022f18227f291bc51dbe48d0ca`
- created_at_utc: `2026-04-22T18:02:57.342381+00:00`
- updated_at_utc: `2026-04-22T18:02:57.342381+00:00`
