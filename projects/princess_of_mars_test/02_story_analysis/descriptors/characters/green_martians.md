# Character Descriptor: green_martians

- descriptor_id: `DESC_CHAR_green_martians`
- canonical_id: `green_martians`
- status: `canonical`
- entity_type: `character`

## Supported Fields

- chapter_mentions:
  - CH004
- distinctive_features:
  - green skin
  - green pigmentation
- entity_kind: group
- identity_baseline: extraterrestrial group characterized by green skin.
- origin_or_historical_context: Martian setting.
- physical_presence_notes: extraterrestrial group characterized by green skin. readable production detail group silhouette emphasizes numbers and hardiness coordinated, military group movement group presence reads as organized and formidable
- silhouette_notes: group silhouette emphasizes numbers and hardiness extraterrestrial group characterized by green skin.

## Generated Fields

- age_presence: readable production detail
- age_range: adult fighting-age group
- build: varied but generally battle-hardened builds
- costume_materials: repeatable martial fabrics, leathers, and gear materials
- expression_tendency: alert, battle-ready expressions
- eye_color: varied but species-consistent eyes
- face_shape: varied face shapes across the group
- facial_hair: mixed or minimal facial hair across the group
- hair_color: varied hair coloring where applicable
- hair_style: utilitarian grooming suited to the group role
- height: varied heights within a broad martial group
- movement_language: coordinated, military group movement
- physical_build: group silhouette emphasizes numbers and hardiness
- posture: disciplined group-ready posture
- sex: mixed or unspecified group
- skin_tone: group-consistent skin tones matching their species and region
- voice_or_presence_notes: group presence reads as organized and formidable

## Coverage

- chapter_mentions: CH004
- scene_mentions: (none)
- shot_mentions: (none)

## Evidence Summary

- A group of extraterrestrial entities identified by their distinct green skin. Further visual details regarding build, clothing, or movement are not provided in the available evidence.
- green pigmentation
- [CH003] CHAPTER III (book index hit)
- [CH020] CHAPTER XX (book index hit)
- [CH021] CHAPTER XXI (book index hit)

## Review

- (none)

## Metadata

- artifact_id: `DESC_CHAR_green_martians`
- status: `generated`
- source_fingerprint: `0848c54527dda9f7155a0926d529e65ab4f7910f`
- created_at_utc: `2026-04-23T05:37:41.936933+00:00`
- updated_at_utc: `2026-04-23T05:37:41.936933+00:00`
