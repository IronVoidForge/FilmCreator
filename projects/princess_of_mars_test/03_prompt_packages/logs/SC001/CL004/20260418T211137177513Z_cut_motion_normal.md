...
- timestamp_utc: 2026-04-18T21:11:37.177513+00:00
- stage: cut_motion
- clip_id: CL004
- retry_kind: normal
...
Generate cut motion video for close-up examination of banner designs and devices on Martian airship prows, transitioning from wide fleet formation to detailed focus while maintaining midday lighting continuity and narrator observation eyeline shift
...
Close-up shot of strange banners on gray airship prows, odd devices visible on vessel fronts, green-skinned warrior figures static on selected ships, hill crests providing depth context in background, midday sunlight reflecting off metallic ship surfaces, camera slight push-in from wide formation to banner detail focus, narrator eyeline shift from fleet-wide view to individual ship examination, banners showing mysterious symbols and patterns, devices appearing as strange mechanical objects on prows, atmospheric tension maintained throughout motion
...
morphing faces, flickering lights, extra limbs, distorted anatomy, sudden jumps, static image, wrong character count, blurry text, inconsistent lighting, background shifting, glitchy movement, incorrect character identity, interior corridor space, empty room transition, fleet appearing too fast, morning light intrusion, banner design changes, device shape distortion, camera push too abrupt, warm interior shadows breaking continuity
...
- clip_id: CL004
- duration_seconds: 5
- required_refs: Beat BT003.md, Scene Summary, Chapter VIII daylight conditions
- optional_refs: Close-up banner detail focus, Over-the-shoulder with focus pull to specific ships, Hill crests depth context for close-ups
- visible_character_assets: Green Warriors static on selected vessels, Gray airships with visible prows and devices, Banner designs continuity markers
- look_continuity_policy: Midday Light Established - Midday light fully established by end of beat
- intended_lighting_change: None - maintain midday lighting consistency from keyframe
- composition_type: wide_formation_to_close_up_detail
- continuity_mode: reblock_same_scene
- starting_keyframe_strategy: Fleet in wide formation visible from observation point, banner designs partially identifiable
- dependency_policy: dependent_on_fleet_position_and_banner_visibility
- auto_advance_policy: maintain_banner_design_continuity
- fallback_strategy: cutaway_to_device_shape_detail
- consistency_assist_policy: preserve_banner_symbol_integrity
- consistency_assist_method: match_banner_pattern_across_frames
- anatomy_repair_policy: regenerate_with_stricter_constraints_if_distortion_detected
- consistency_targets: banner_design_markers, device_shapes, ship_prow_details, lighting_grade
- style_profile: sci_fi_adventure_war_observation_aesthetic
- batch_role: detail_focus_clip_in_retreat_sequence
- fix_of: SC001_CL004_cut_motion_prompt
- workflow_type: video.cut_motion.wan.i2v
...
- Match keyframe lighting and grade by default from approved opening frame
- Preserve interior shadows and depth cues from observation point window framing
- Ensure banner design continuity markers remain identifiable across motion sequence
- Maintain character consistency with previous clips in scene (ship count, positioning)
- Keep narrator eyeline shift smooth from wide formation to close detail focus
- Hill crests depth context must stay consistent for spatial reference
- Banner symbols and patterns should not change or morph during cut motion
...
- If banner designs appear to morph, regenerate with stricter pattern constraints
- If lighting shifts from midday grade, revert to base keyframe lighting
- If camera push is too abrupt, smooth out transition speed
- If ship count changes in frame, maintain twenty ships in wide shots
- If device shapes distort on close-up, use fallback cutaway strategy
- If interior shadows break continuity, restore observation point window framing
