...
from .scaffold import create_clip, create_project, create_run_manifest, create_scene
...
def _route_outputs(prepared: PreparedRun, comfy_outputs: list[dict[str, str]]) -> list[Path]:
    preferred_output_nodes = {str(node_id) for node_id in prepared.workflow.get("preferred_output_nodes", [])}
    selected_outputs = comfy_outputs
    if preferred_output_nodes:
        selected_outputs = [output for output in comfy_outputs if str(output.get("node_id")) in preferred_output_nodes]
        if not selected_outputs:
            available = ", ".join(sorted({str(output.get("node_id")) for output in comfy_outputs}))
            raise RuntimeError(
                "ComfyUI completed, but none of the outputs matched the preferred output nodes "
                f"for workflow '{prepared.workflow['id']}'. Available output nodes: {available or 'none'}"
            )

    output_dir = prepared.output_root
    ensure_dir(output_dir)

    routed: list[Path] = []
    start_index = _next_index(output_dir, ASSET_CODE_BY_STAGE.get(prepared.stage))
    version = _next_version(output_dir)

    for output_number, output in enumerate(selected_outputs, start=1):
        source = _resolve_comfy_output_source(prepared, output)
        if not source.exists():
            raise FileNotFoundError(f"ComfyUI reported output file that does not exist: {source}")

        if prepared.output_scope == "project":
            if not prepared.asset_id:
                raise ValueError("Project-scoped output routing requires asset_id")
            filename = prepared.output_name_pattern.format(
                asset_id=prepared.asset_id,
                stage=prepared.stage,
                variant=f"{output_number:02d}",
                version=version,
            )
        else:
            if not prepared.scene_id or not prepared.clip_id:
                raise ValueError("Clip-scoped output routing requires scene and clip IDs")
            filename = prepared.output_name_pattern.format(
                scene_id=prepared.scene_id,
                clip_id=prepared.clip_id,
                asset_code=ASSET_CODE_BY_STAGE[prepared.stage],
                index=start_index + output_number - 1,
                version=version,
            )

        destination = output_dir / Path(filename).with_suffix(source.suffix or ".png")
...
def _stage_input_refs(prepared: PreparedRun) -> None:
...
def _cleanup_staged_files(prepared: PreparedRun, comfy_outputs: list[dict[str, str]]) -> None:
...
def _resolve_comfy_output_source(prepared: PreparedRun, output: dict[str, str]) -> Path:
