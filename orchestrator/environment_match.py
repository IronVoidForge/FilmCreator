...
class EnvironmentMatchCandidate:
    canonical_id: str
    display_name: str
    score: int
    reasons: list[str]
...
def find_environment_match_candidates(
    *,
    project_slug: str,
    asset_id: str,
    markdown: str,
    top_n: int = 3,
) -> list[EnvironmentMatchCandidate]:
    registry = _load_environment_registries(project_slug)
    if not registry:
        return []

    query_tokens = _query_tokens(asset_id=asset_id, markdown=markdown)
    candidates: list[EnvironmentMatchCandidate] = []
    for canonical_id, entry in registry.items():
        ...
        context_snippets = _source_context_snippets(
            ...
            query_terms=[asset_id, str(entry.get("display_name", ""))],
        )
        candidates.append(
            EnvironmentMatchCandidate(
                canonical_id=canonical_id,
...
def _load_environment_registries(project_slug: str) -> dict[str, dict]:
    registries: dict[str, dict] = {}
    registry_paths = [
        global_environment_registry_path(project_slug),
        create_project(project_slug) / "02_story_analysis" / "world" / "ENVIRONMENT_REGISTRY.json",
    ]
...
def _query_tokens(*, asset_id: str, markdown: str) -> set[str]:
