...
def _looks_like_entity_association(text: str) -> bool:
    ...
    association_patterns = [
        r"\(nearby\)", r"\(near\)", r"nearby", r"next to",
        r"standing beside", r"associated with", r"accompanied by",
        r"with\s+[A-Z][a-z]+", r"protagonist", r"character", r"companion",
    ]
...
def _filter_entity_associations(values: list[str]) -> list[str]:
    ...
    return [v for v in values if not _looks_like_entity_association(v)]
...
def _is_generic_group_filler(field_name: str, value: Any) -> bool:
    ...
    generic_patterns = [
        r"varied\s+(hair|eye|face|skin)", r"group-consistent\s+",
        r"where\s+applicable", r"species-consistent",
        r"mixed\s+or\s+(minimal|unspecified)", r"across\s+the\s+group",
    ]
...
def _should_suppress_group_field(field_name: str, value: Any, supported_values: dict[str, Any]) -> bool:
    ...
    evidence_required_fields = {"hair_color", "eye_color", "face_shape", "skin_tone", "facial_hair"}
...
Before writing each generated field, silently check that the value makes sense for this exact entity: it must agree with the identity locks, pronoun/name cues, species or entity kind, age/social role, costume evidence, movement evidence, and the other generated fields. Do not fill a field with the first plausible generic value; choose a production-usable value that is internally consistent with the whole descriptor, or add a field-specific review flag when support is weak.
Generated fields should read like deliberate casting/design choices for this character or environment, not a reusable default bundle. Avoid adult/male/human portrait assumptions unless they are supported by the identity context. Avoid modern clothing, modern grooming, or unrelated genre details unless the source clearly supports them.
...
Before writing each missing generated field, silently check that the value makes sense for this exact entity: it must agree with the identity locks, pronoun/name cues, species or entity kind, age/social role, costume evidence, movement evidence, and the other generated fields. Do not fill a field with the first plausible generic value; choose a production-usable value that is internally consistent with the whole descriptor, or add a field-specific review flag when support is weak.
Generated fields should read like deliberate casting/design choices for this character or environment, not a reusable default bundle. Avoid adult/male/human portrait assumptions unless they are supported by the identity context. Avoid modern clothing, modern grooming, or unrelated genre details unless the source clearly supports them.
