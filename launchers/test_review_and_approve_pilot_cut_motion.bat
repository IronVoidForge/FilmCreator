...
if not exist "%FILMCREATOR_ROOT%\%CLIP_STATE_REL%" (
    echo Missing clip state: %FILMCREATOR_ROOT%\%CLIP_STATE_REL%
    exit /b 1
)

if not exist "%FILMCREATOR_ROOT%\%VIDEO_DIR_REL%" (
    echo Missing video directory: %FILMCREATOR_ROOT%\%VIDEO_DIR_REL%
    exit /b 1
)
...
if not defined MANIFEST_REL (
    echo No latest cut-motion manifest was found in clip state.
    echo Run the short-cut cut-motion batch BAT first.
    exit /b 1
)

if not exist "%FILMCREATOR_ROOT%\%MANIFEST_REL%" (
    echo Missing cut-motion manifest: %FILMCREATOR_ROOT%\%MANIFEST_REL%
    exit /b 1
)

for /f %%I in ('powershell -NoProfile -ExecutionPolicy Bypass -Command "(Get-ChildItem \"%FILMCREATOR_ROOT%\%VIDEO_DIR_REL%\" -File | Where-Object { $_.Extension -match ''^\.(mp4|mov|webm|mkv)$'' -and $_.Name -match ''_v\d+'' } | Measure-Object).Count"') do set "VIDEO_COUNT=%%I"
if not "%VIDEO_COUNT%"=="4" (
    echo Expected exactly 4 video candidates, but found %VIDEO_COUNT%.
    echo Check the video folder before running this approval test.
    exit /b 1
)
...
echo.
echo Review the 4 short video candidates, then choose your top 2 finalists and 1 primary winner.
echo.
...
if errorlevel 1 (
    echo.
    echo review-batch failed. No video promotion was performed.
    pause
    exit /b 1
)

python -m orchestrator promote-asset %PROJECT_SLUG% "%PRIMARY_PATH%" approved_video --scene %SCENE_ID% --clip %CLIP_ID% --index 1
if errorlevel 1 (
    echo.
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "Write-Host ('approved_video: ' + $clipState.approved_assets.approved_video); " ^
  "Write-Host ('approved_video_last_frame: ' + $clipState.approved_video_last_frame); " ^
...
:print_candidates
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$files = Get-ChildItem '%FILMCREATOR_ROOT%\%VIDEO_DIR_REL%' -File | Where-Object { $_.Extension -match '^\.(mp4|mov|webm|mkv)$' -and $_.Name -match '_v\d+' } | Sort-Object Name; " ^
...
for /f "usebackq delims=" %%I in (`powershell -NoProfile -ExecutionPolicy Bypass -Command "$files = Get-ChildItem '%FILMCREATOR_ROOT%\%VIDEO_DIR_REL%' -File | Where-Object { $_.Extension -match '^\.(mp4|mov|webm|mkv)$' -and $_.Name -match '_v\d+' } | Sort-Object Name; $index = %CANDIDATE_INDEX%; if($index -lt 1 -or $index -gt $files.Count){ throw 'Candidate index out of range.' }; $files[$index - 1].FullName"`) do set "%TARGET_PATH_VAR%=%%I"
for /f "usebackq delims=" %%I in (`powershell -NoProfile -ExecutionPolicy Bypass -Command "$files = Get-ChildItem '%FILMCREATOR_ROOT%\%VIDEO_DIR_REL%' -File | Where-Object { $_.Extension -match '^\.(mp4|mov|webm|mkv)$' -and $_.Name -match '_v\d+' } | Sort-Object Name; $index = %CANDIDATE_INDEX%; if($index -lt 1 -or $index -gt $files.Count){ throw 'Candidate index out of range.' }; $files[$index - 1].Name"`) do set "%TARGET_NAME_VAR%=%%I"
exit /b 0
